...
const MESSAGE_SUBSTITUTION_PATTERN = /\{\{([\w.]+)\}\}/g;
...
var Translator = module.exports = function Translator(options, callback) {
  ...
  this.__internal.substitutionPattern = options.substitutionPattern || MESSAGE_SUBSTITUTION_PATTERN;
...
function translateMessage(msgid, options) {
  ...
  if (options && options.data) {
    msgtext = substituteTokens(msgtext, options.data);
  }
...
*/
function substituteTokens(message, data) {
  ...
  return message.replace(messageSubstitutionPattern, function(match, token) {
